...
//
// Copyright 2025, J.C.L. (Hans) Meeussen
//
...
void test(string name, vector<Node*>* nodes, NodeProcessor* np, FileWriter* report, FileBasket fileBasket, ParameterList* outputVariableNames, vector<int>* outputIndx, int memoryOption, bool writeOutput) {
	...
	//int failedindex = nodes->at(0)->nodeType->index("failed");
	...
	for (int n = 0; n < nodes->size(); n++) {
		tot_nr_iter += nodes->at(n)->getvalue(iterindex);
	}
	...
	for (int n = 0; n < nodes->size(); n++) {
		if (nodes->at(n)->getvalue(iterindex) < 0.5) {
			//std::cout << n << " :1:  " << nodes->at(n)->getvalue(iterindex) << endl;
			nrFailedNodes++;
		}
	}
	...
	if (writeOutput) {
		...
		FileWriter fw("output_" + name + ".txt");
		// write header 
...
//		OrchestraReader* inputReader = OrchestraReader::getOrchestraFileReader(&fileBasket, "input.dat");
		...
		// An ORCHESTRA parameterlist is a helper class that reads a list of comma or space tab separated strings form a line into a list of parameters
		// we use it here to read the header line as well as the subsequent data points from the input file
		...
		do {
			...
			ParameterList inputDataLine(line);
		...
		for (int i = 0; i < nodes.size(); i++) {
			nodes_random_single.push_back(nodes.at(i)->clone()); // this creates new nodes
...
//		FileWriter* report = fileBasket.getFileWriter(&fileBasket, "report.txt");
		...
		// now we can do a test with increasing number of threads
		...
		for (int nrThreads = 1; nrThreads <= 32; nrThreads=nrThreads*2) {
        	...
        	// we use a fresh copy of the original node values, otherwise the results of the previous calculation will be present in the nodes and speed up the calculations
			for (int i = 0; i < nodes.size(); i++) {
				nodes_random_multi.at(i)->clone(nodes.at(i)); // so we copy the contents of node into node_random_multi
			}
